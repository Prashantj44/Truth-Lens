...
def run_tests():
    ...
    # 1. Health check
    r = client.get("/api/index.py", headers={"x-matched-path": "/api/health"})
    ...
    r = client.get("/api/index.py", headers={"x-matched-path": "/api/news/status"})
    ...
    # 3. Trending Claims
    r = client.get("/api/index.py", headers={"x-matched-path": "/api/news/trending"})
    ...
    # 4. History
    r = client.get("/api/index.py", headers={"x-matched-path": "/api/history"})
    check("History Endpoint 200", r.status_code == 200)

    # 5. Analytics
    r = client.get("/api/index.py", headers={"x-matched-path": "/api/analytics"})
    check("Analytics Endpoint 200", r.status_code == 200)

    # 6. Verification: Truthful Claim
    r = client.post("/api/index.py", headers={"x-matched-path": "/api/verify"}, json={"claim": "The earth revolves around the sun"})
    ...
    r = client.post("/api/index.py", headers={"x-matched-path": "/api/verify"}, json={"claim": "Donald Trump is the Prime Minister of India"})
